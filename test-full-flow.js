...
// Configuration
const PROGRAM_ID = new web3_js_1.PublicKey('CMrfhDiq5gLBwbT3nxQvGH1UTcxtaAY9r4we2PvjbjAg');
const BASE_MINT = new web3_js_1.PublicKey('yXJUy2a1YgKDJ5CfngRN7djwX3Dtbv85f9jUFCgutdj');
const QUOTE_MINT = new web3_js_1.PublicKey('4eYgX7VZj4eQ5Vf5MbmzCgAwcbhkP1rSMhR5jZmdZN5H');
...
async function main() {
    ...
    const secretKey = JSON.parse((0, fs_1.readFileSync)(walletPath, 'utf-8'));
    const wallet = web3_js_1.Keypair.fromSecretKey(new Uint8Array(secretKey));
    ...
    // Derive PDAs
    const [marketPda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('market'), BASE_MINT.toBuffer(), QUOTE_MINT.toBuffer()], PROGRAM_ID);
    const [configPda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('config')], PROGRAM_ID);
    const [baseVaultPda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('vault'), Buffer.from('base'), BASE_MINT.toBuffer(), QUOTE_MINT.toBuffer()], PROGRAM_ID);
    const [quoteVaultPda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('vault'), Buffer.from('quote'), BASE_MINT.toBuffer(), QUOTE_MINT.toBuffer()], PROGRAM_ID);
    console.log('\n📦 PDAs:');
    ...
    console.log('='.repeat(60));
    try {
        const baseTokenAccount = await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, wallet, BASE_MINT, wallet.publicKey);
        console.log('✅ Base Token Account:', baseTokenAccount.address.toString());
        console.log('   Balance:', baseTokenAccount.amount.toString());
        const quoteTokenAccount = await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, wallet, QUOTE_MINT, wallet.publicKey);
    ...
    try {
        ...
        const [bidOrderPda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('order'), marketPda.toBuffer(), wallet.publicKey.toBuffer(), bidOrderId.toArrayLike(Buffer, 'le', 8)], PROGRAM_ID);
        const bidTx = await program.methods
            .placeOrder(bidOrderId, { bid: {} }, new anchor.BN(100000000), // 100 quote tokens
        new anchor.BN(9000000) // want at least 9 base tokens
        )
            .accounts({
            order: bidOrderPda,
            market: marketPda,
        ...
        const [askOrderPda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('order'), marketPda.toBuffer(), wallet.publicKey.toBuffer(), askOrderId.toArrayLike(Buffer, 'le', 8)], PROGRAM_ID);
        const askTx = await program.methods
            .placeOrder(askOrderId, { ask: {} }, new anchor.BN(10000000), // 10 base tokens
        new anchor.BN(95000000) // want at least 95 quote tokens
        )
            .accounts({
            order: askOrderPda,
            market: marketPda,
    ...
    try {
        ...
        const healthResponse = await (0, node_fetch_1.default)(`${SOLVER_URL}/health`);
        ...
        const matchResponse = await (0, node_fetch_1.default)(`${SOLVER_URL}/match-and-settle`, {
            method: 'POST',
    ...
    try {
        ...
        const baseTokenAccount = await (0, spl_token_1.getAccount)(connection, (await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, wallet, BASE_MINT, wallet.publicKey)).address);
        const quoteTokenAccount = await (0, spl_token_1.getAccount)(connection, (await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(connection, wallet, QUOTE_MINT, wallet.publicKey)).address);
